...
function renderSDEMetrics() {
  ...
  // Time from ticket creation to resolution plan being set (MAC + Reactive queues)
  const reactiveAndMacClosed = [...reactiveClosedList, ...macClosedList].filter(t => t.workedHours > 0);
  const ticketsWithResponsePlan = reactiveAndMacClosed.filter(t => t.createDate && t.resolutionPlanDateTime);
  ...
  // Time from ticket creation to resolution (MAC + Reactive queues)
  const ticketsWithResolved = reactiveAndMacClosed.filter(t => t.createDate && t.resolvedDateTime);
